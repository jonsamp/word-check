...
export function DictionaryContextMenu({
  ...
  const menuLabel = (
    <Button modifiers={[buttonStyle("bordered")]}>
      <HStack spacing={8}>
        ...
        <SwiftUIText modifiers={[font({ design: "serif" }), foregroundStyle(color)]}>
  ...
  const buttons = (Object.keys(DictionaryNames) as Dictionary[]).map((dictionaryId) => {
    ...
    return (
      <Button key={dictionaryId} onPress={() => onSelectDictionary(dictionaryId)}>
